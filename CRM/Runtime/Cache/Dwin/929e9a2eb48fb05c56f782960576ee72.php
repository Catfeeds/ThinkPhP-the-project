                ...
                <div class="title">
                        ...
                        <div>
                            ...
                            <!-- <button class="btn btn-xs btn-outline btn-success audit_staff"><span class="glyphicon glyphicon-adjust"></span>审核</button> -->
                             <button class="btn btn-xs btn-outline btn-success edit_staff"><span class="glyphicon glyphicon-edit"></span>下推订单</button>
                            <!--<button class="btn btn-xs btn-outline btn-success details_staff"><span class="glyphicon glyphicon-align-justify"></span>物料订单详情</button>
                            <button class="btn btn-xs btn-outline btn-success delete_staff"><span class="glyphicon glyphicon-remove"></span>删除物料订单信息</button>
                    ...
                    <div role="tabpanel" class="tab-pane active" id="contact">
                        <table class="table table-striped table-hover table-border">
                            ...
                            <tr v-for="item in contact">
                                ...
                                <td>{{item.num  || ''}}</td>
                                <td>{{item.default_rep_id  || ''}}</td>
                                <td>{{item.fail_rep_id  || ''}}</td>
                            </tr>
                        </table>
                    </div>
                </div>
...
<script>
    var table = $('#staff'). DataTable({
        ...
        // order:[[21, 'desc']],
        columns: [
            {searchable: true, data: 'stock_in_id'},
            {searchable: true, data: 'supplier_name'},
            {searchable: true, data: 'purchase_mode'},
            {searchable: true, data: 'purchase_type'},
            {searchable: false, data: 'auditor_name'},
            {searchable: false, data: 'pay_time'},
            {searchable: true, data: 'keep_name'},
            {searchable: true, data: 'other_bill'},
            {searchable: true, data: 'check_name'}
        ],
        oLanguage: {
            "oAria": {
    ...
    $('tbody').on('click', 'tr', function () {
        ...
        $.post('/Dwin/Stock/getStockInMaterial', {id: orderId}, function (res) {
            vm.contact = res.data
        })
    })
    var vm = new Vue({
    ...
    $('.edit_staff').on('click', function () {
        if (order_id === undefined){
            ...
            var index = layer.open({
                type: 2,
                title: '湖南迪文科技有限公司入库单下推',
                content: '/Dwin/Stock/addRecordWithPurchaseStockIn?id=' + orderId,
                area: ['90%', '90%'],
                shadeClose:true,
                end: function () {
                    table.ajax.reload()
