...
<script>
    ...
    function getHtml(data)
    {
        ...
        if (data.order_num_all != 0) {
            var totalM = parseFloat((parseFloat(data.cartography_fee) + parseFloat(data.maintenance_fees) + parseFloat(data.settle_research_price) + parseFloat(data.settle_normal_price)),2);
            spanHtml += "订单总数量：" + data.order_num_all + ";价值业绩：" + parseFloat(data.value_price) + "; 拓展业绩：" + parseFloat(data.marketing_price) + "; 研发费：" + parseFloat(data.settle_research_price);
            spanHtml += "; 制图费：" + parseFloat(data.cartography_fee) + "; 维修费：" + parseFloat(data.maintenance_fees) + "; 业绩总额：" + totalM + "; 总出货量：" + data.product_nums;
        }
        $("#statistics").html("");
            ...
            oTables3 = dataTableOrderList3Div.dataTable({
                ...
                "ajax"         : {  //获取数据
                    ...
                    "data"  : {
                        ...
                        "deptLimit" : function () {
                            return document.getElementById('deptId3').value;
                        },
                        "timeLimit1" : function () {
    ...
    deptSel3.on('change', function (){
        // 业务员下拉里列表变化
        changeSel($(this).val(), staffSel, dataTableOrderListDiv);
        // 数据表格更新
    });
