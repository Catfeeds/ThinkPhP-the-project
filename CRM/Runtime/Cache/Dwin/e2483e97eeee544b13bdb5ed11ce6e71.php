                                            ...
                                            <tr v-for="(item,data_key) in batch.data">
                                                ...
                                                <td>
                                                    <select v-if="!item.id" v-validate="'required'" class="form-control" v-model="item.warehouse_number" :disabled="item.audit_status == 2 || item.audit_status == 1 || !stockEditAble" @change="warehouseSelect(batch_key, data_key)">
                                                        ...
                                                        <?php if(is_array($repMap)): $i = 0; $__LIST__ = $repMap;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vol["rep_id"]); ?>"><?php echo ($vol["repertory_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                                    </select>
...
<script>
    Vue.use(VeeValidate);
    var repairItemList   = <?php echo (json_encode($repairProductData)); ?>; // 维修品基本信息
    var product_list     = <?php echo (json_encode($productList)); ?>;  // 维修品具体维修信息
    var feeAndTips       = <?php echo (json_encode($saleRecordBasicData[0])); ?>;    // 反馈与人工费
    var repairStaff      = <?php echo (json_encode($repairStaffArray)); ?>;   // 维修员列表
    var salesmanFeedback = <?php echo (json_encode($saleRecordBasicData)); ?>;  //业务反馈信息
    var sendGoods        = <?php echo (json_encode($sendGoods)); ?>;   // 物流信息
    var re_status_list   = JSON.parse(<?php echo (json_encode($repairStatus)); ?>);    // 维修状态列表
    var re_mode_list     = JSON.parse(<?php echo (json_encode($repairMode)); ?>);     // 维修方式列表
    var reperson_question_list = JSON.parse(<?php echo (json_encode($repairQuestion)); ?>);   // 故障类型列表
    var auditorArr       = <?php echo (json_encode($auditorArr)); ?>;     // 出入库审核员列表
    var stock_io_record  = <?php echo (json_encode($stock_io_record)); ?>;     // 出入库记录
    var pid  = <?php echo ($sid); ?>;     // 出入库记录
     ...
     * */
    var editableFlag     = <?php echo (json_encode($flagArray)); ?>;
        ...
        methods: {
                    ...
                    if (res === true){
                        ...
                        var index = layer.confirm('是否完结此单?', {title: '是否完结',btn:['完结', '保存']},
                            function () {
                                obj.flag = 1;
                                // console.log(obj);
                                $.post('<?php echo U("SaleService/editSaleRepairing");?>', obj, function (res) {
            ...
            addStockBatch: function () {
                ...
                for (var i = 0; i < productList.length; i++){
                    data = {
                        ...
                        proposer: "<?php echo session('staffId');?>",
                        proposerArr: "<?php echo session('staffId');?>_<?php echo session('nickname');?>",
            ...
            openPrintPage: function () {
                layer.open({
                    type: 2,
                    title: '打印详情',
                    content: "<?php echo U('printSalerecord');?>?sid="+pid,
                    area: ['100%', '100%'],
