...
class PurchaseSupplierCustomerModel extends Model
{
    ...
    protected $_validate = array(
        array("cus_name","require","客户名称不能为空!"),
        array("main_project","require","项目类型不能为空!"),
        array("main_contact","require","联系人不能为空!"),
        array("main_phone","require","联系方式不能为空!"),
        array("project_exec_time","require","项目实施时间不能为空!"),
        array("project_amount","require","项目实施时间不能为空!"),
    );
    ...
    public function getAddData($params)
    {
        ...
        //数据非空验证
        if(empty($data['cus_name']) || empty($data['main_project']) || empty($data['main_contact']) || empty($data['main_phone']) || empty($data['project_exec_time']) || empty($data['project_amount'])){
            return [-2, [], "请将数据填写完成"];
        }
    ...
    public function getCustomerWithPId($supplierId)
    {
        $map['is_del'] = ['EQ', self::$notDel];
        $map['supplier_pid'] = ['EQ', $supplierId];
        return $this
            ->field("crm_purchase_supplier_customer.*,crm_staff.name")
            ->join("left join crm_staff on crm_staff.id = crm_purchase_supplier_customer.create_id")
            ->where($map)
            ->select();
    }
}